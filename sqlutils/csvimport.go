...
func NewCSVImporter( schema string ) *CSVImporter {
  return &CSVImporter{schema:schema}
}

func (a *CSVImporter) Name() string {
  return "CSVImporter:" + a.schema
}
...
// CSVImport handles the importing of CSV data into a table.
// If the inbound data has a pair of columns called "Easting","Northing" or
// "Longitude","Latitude" then it will also handle populating a geom column
// with a point, using EPSG:2700 or EPSG:4326 accordingly.
